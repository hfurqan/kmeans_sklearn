...
def _k_init(X, n_clusters, x_squared_norms, random_state, n_local_trials=None):
    ...
    closest_dist_sq = pairwise_distances(centers[0, np.newaxis], X, 
                           metric = LB_Keogh)
    #closest_dist_sq = manhattan_distances(
        #centers[0, np.newaxis], X)
    #closest_dist_sq = closest_dist_sq * closest_dist_sq
    ...
    for c in range(1, n_clusters):
        ...
        distance_to_candidates = pairwise_distances(X[candidate_ids], X, 
                           metric = LB_Keogh)
        #distance_to_candidates = manhattan_distances(
            #X[candidate_ids], X)
        #distance_to_candidates = distance_to_candidates * distance_to_candidates
...
def LB_Keogh(s1,s2):
    ...
    for ind,i in enumerate(s1):
        ...
        lower_bound=min(s2[(ind-10 if ind-10>=0 else 0):(ind+10)])
        upper_bound=max(s2[(ind-10 if ind-10>=0 else 0):(ind+10)])
...
pl_df = pd.read_csv('D://Transaction Pattern - Risk//Processed Data//personal_loan_data_normalized.csv')
X = pl_df.loc[:,"1":"90"].values
Xi = X[0:1000,0:90]
...
plt.show()
t1 = t.time()
print("Time taken by elbow method: ",t1-t0)
...
t0 = t.time()
kmeansbatch = MiniBatchKMeans(n_clusters=10,
                         random_state=123,
                         batch_size=500000)

y_kmeans = kmeansbatch.fit_predict(X)
t1 = t.time()
...
for i in range(1, 10):
    ...
    kmeansbatch = MiniBatchKMeans(n_clusters=10,
                         ...
                         batch_size=200000)
    kmeansbatch.fit(Xi)
    wcss.append(kmeansbatch.inertia_)
